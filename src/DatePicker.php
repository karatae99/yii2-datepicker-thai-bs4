...
class DatePicker extends InputWidget
{
     ...
     */
    public $addon = '<i class="fas fa-calendar-alt"></i>';
    ...
    public function run()
    {
        ...
        if ($this->addon && !$this->inline) {
            $sep = ['class' => 'input-group-addon'];            
            $css = $this->template === '{input}{addon}' ? 'append' : 'prepend';
            $options = 'input-group-' . $css;
            Html::addCssClass($sep, ['input-group-text', $options]);           
            $addon = Html::tag('span', $this->addon, $sep);           
