...
class DateRangePicker extends InputWidget
{
     ...
     */
    protected $_template = '{inputFrom}{labelTo}{inputTo}';
    ...
    {
        if ($this->form) {
            Html::addCssClass($this->options, [$this->addInputCss, 'datepicker-from']);
            Html::addCssClass($this->optionsTo, [$this->addInputCss, 'datepicker-to']);
            $css ='form-control';
            $inputFrom = $this->form->field(
                ...
                [
                    ...
                    'options'  => ['class' => "datepicker-range {$this->addInputCss}"],
                ]
            ...
            Html::addCssClass($this->optionsTo, $this->addInputCss);
        ...
        $labt = Html::tag('span', $this->labelTo, ['class' => 'input-group-text']);        
        $sep = Html::tag('div', $labt, ['class' => 'input-group-append']);
       
        echo Html::tag(
            'div',
            strtr(
                $this->_template,
                ['{inputFrom}' => $inputFrom, '{labelTo}' => $sep, '{inputTo}' => $inputTo]
